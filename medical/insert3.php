<?php
	include "connection.php";	
		...
		$location="../assets/images/" . $_FILES["image"]["name"];
				$query = "INSERT INTO `user_master`(`login_id`, `fname`, `lname`, `email`, `phone_no`, `dob`, `password`, `role_id`, `active`, `status`, `gender`, `address`, `profile_pic`, `blood_group`, `state`, `city`) VALUES ('','$name','$lname','$email','$phone','$date','$pass1','3','1','0','$gender','$address','$location','$bg','$state','$city')";
				$result = mysqli_query($con,$query);
				$s="SELECT login_id FROM user_master WHERE email='$email'";
				$a=mysqli_query($con,$s);
				$q=mysqli_fetch_array($a);
				$p=$q['login_id'];
				$query1 = "INSERT INTO `medical_master`(`medical_id`, `login_id`, `license_number`, `qualification`) VALUES ('','$p','$lic','$qualify')";
